...
router.get('/:id', verifyToken, async (req, res) => {
  try {
    ...
    // Get all users with their bidding status and days since last recall
...
router.post('/:id/bid', verifyToken, async (req, res) => {
  try {
    const { id } = req.params;
    const { response } = req.body; // 'bid', 'not_available', or 'not_bid'
    ...
    // Check if recall exists and is active
    const [recalls] = await pool.execute(
    ...
    // Check if recall is already assigned
    const [assignments] = await pool.execute(
...
router.post('/:id/award', verifyToken, async (req, res) => {
  try {
    ...
    // Check if user has bid on this recall
  ...
  try {
    ...
    const [history] = await pool.execute(`
      SELECT 
        ...
        ra.hours
      FROM recalls r
      LEFT JOIN recall_responses rr ON r.id = rr.recall_id AND rr.user_id = ?
      LEFT JOIN recall_assignments ra ON r.id = ra.recall_id
      WHERE rr.user_id = ?
